...
static gboolean match_selected (GtkEntryCompletion *completion,
				...
				GtkTreeIter        *iter,
				gpointer            entry);
static void _no_suggestions (GtkEntryCompletion *completion, GWeatherLocationEntry *entry);
...
gweather_location_entry_init (GWeatherLocationEntry *entry)
{
    ...
    */
    ...
    g_signal_connect (completion, "no-suggestions",
                      G_CALLBACK (_no_suggestions), entry);
...
entry_changed (GWeatherLocationEntry *entry)
{
    ...
    if (!g_cancellable_is_cancelled (entry->priv->cancellable)) {
             ...
             gpointer      user_data)
{
    ...
    if (places != NULL) {
        ...
        if (GTK_IS_ENTRY_COMPLETION (user_data)) {
            gtk_entry_completion_set_model (user_data, GTK_TREE_MODEL(store));
        }
        g_object_unref (store);
    }
    if (GTK_IS_ENTRY_COMPLETION (user_data)) {
        gtk_entry_completion_delete_action (user_data, 0);
        gtk_entry_completion_set_match_func (user_data, matcher, NULL, NULL);
    }
}

static void
_no_suggestions (GtkEntryCompletion *completion, GWeatherLocationEntry *entry) {
    ...
    geocode_forward_search_async (forward, entry->priv->cancellable, _got_places, completion);
}
